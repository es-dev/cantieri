    ...
    public class DashboardLavoroViewModel : TemplateViewModel<DashboardLavoro, DashboardLavoroItem>
    {
        ...
        private IQueryable<DashboardLavoro> QueryDashboards()
        {
            try
            {
                ...
                var dashboardFatturaAcquisto = new DashboardLavoro("Fatture di acquisto", "FA", "Inserisci le fatture di acquisto per i fornitori, specifica i prodotti acquistati e i costi relativi...", "Images.dashboard.fatturaacquisto.png", "", typeof(FatturaAcquistoView));
                var dashboardArticolo = new DashboardLavoro("Articoli", "ART", "Inserisci gli articoli per le fattura di acquisto, specifica le quantità, i costi e gli eventuali sconti relativi...", "Images.dashboard.articolo.png", "", typeof(ArticoloView));
                var dashboardPagamento = new DashboardLavoro("Pagamenti", "PAG", "Gestisci i pagamenti per le fattura inserite, controlla le scadenze, le passività e gli importi dati e in dare...", "Images.dashboard.pagamento.png", "", typeof(PagamentoView));
                var dashboardNotaCredito = new DashboardLavoro("Note di credito", "NC", "Gestisci le note di credito per i fornitori, specificando gli importi da stornare e le fatture di acquisto...", "Images.dashboard.notacredito.png", "", typeof(NotaCreditoView));
                var dashboardPagamentoUnificato = new DashboardLavoro("Pagamenti unificati", "PU", "Gestisci i pagamenti unificati per i fornitori inseriti. Potrai saldare in un unico pagamento una o più fatture insolute...", "Images.dashboard.pagamentounificato.png", "", typeof(PagamentoUnificatoView));
                var dashboardPagamentoUnificatoFatturaAcquisto = new DashboardLavoro("Pagamenti unificati - fatture", "PU/FA", "Specifica le fatture di acquisto da associare la pagamento unificato. Il sistema valuterà in automatico il saldo da pagare...", "Images.dashboard.pagamentounificatofatturaacquisto.png", "", typeof(PagamentoUnificatoFatturaAcquistoView));
                ...
                var dashboardFatturaVendita = new DashboardLavoro("Fatture di vendita", "FV", "Inserisci le fatture di vendita per i committenti delle commesse, gestisci gli incassi e il conto economico...", "Images.dashboard.fatturavendita.png", "", typeof(FatturaVenditaView));
                var dashboardLiquidazioni = new DashboardLavoro("Incassi", "INC", "Gestisci gli incassi corrispondenti alle fatture di vendita per le commesse inserite nel sistema, visualizza i movimenti in date e in dare...", "Images.dashboard.liquidazione.png", "", typeof(LiquidazioneView));
